...
export default function MobileMenu({ open, items, onClose, brand, cta, activeHref }: Props) {
  ...
  return (
    <motion.aside
      ...
      animate={isMobile ? undefined : open ? "show" : "hidden"}
      variants={isMobile ? undefined : { hidden: { pointerEvents: "none" }, show: { pointerEvents: "auto" } }}
      style={{ display: open ? "block" : "none" }}
      className="fixed inset-0 z-50 md:hidden"
    >
      <motion.div
        variants={isMobile ? undefined : { hidden: { opacity: 0 }, show: { opacity: 1 } }}
      ...
      <motion.div
        variants={isMobile ? undefined : { hidden: { x: "100%" }, show: { x: 0 } }}
        transition={isMobile ? { duration: 0 } : { duration: 0.28, ease: [0.2, 0.8, 0.2, 1] }}
        className="absolute right-0 top-0 h-full w-[84%] max-w-sm border-l border-gray-200 bg-white text-gray-900 p-6 shadow-2xl dark:border-white/10 dark:bg-neutral-950 dark:text-gray-100"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="grid size-9 place-content-center rounded-xl bg-gray-900 text-white text-sm font-semibold dark:bg-white dark:text-neutral-900">
              {brand?.initials ?? "JB"}
            </span>
        ...
        {isMobile ? (
          ...
          <motion.ul variants={staggerContainer(0.06, 0.02)} initial="hidden" animate="show" className="mt-8 space-y-2">
            {items.map((item) => {
              const active = isActive(item.href);
              return (
        ...
        <div className="mt-6 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-white/5">
          <div className="text-xs uppercase tracking-wide text-gray-600 dark:text-neutral-300">Availability</div>
